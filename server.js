...
app.get('/usdc-pairs', async (req, res) => {
    try {
      const exchangeInfo = await binance.exchangeInfo();
      const usdcPairs = exchangeInfo.data.symbols.filter(sym =>
        sym.symbol.endsWith("USDC") && sym.status === "TRADING"
      );
      res.json(usdcPairs.map(sym => sym.symbol));
    } catch (err) {
      console.error("Error fetching USDC pairs:", err);
      res.status(500).json({ error: "Error fetching USDC pairs" });
    }
});
...
function getHistory(fileName, res, historyType) {
	try {
		...
		res
			.status(500)
			.json({ error: `Nem sikerült lekérni a ${historyType} előzményeket` });
	}
}
